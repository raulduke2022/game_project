    ...
    <header class="main-header">
        ...
        <div class="btn-group">
            ...
            <ul class="dropdown-menu p-2">
                <li><a class="dropdown-item mb-1" href="{{ route('orders.index', ['is_done' => 'false']) }}">Новые</a>
                </li>
                <li><a class="dropdown-item mb-1"
                       href="{{ route('orders.index', ['is_done' => 'true']) }}">Обработанные</a></li>
...
{{--                                                        </a>--}}
                            <form action="{{ route('orders.toggle', $order->id) }}" method="POST"
                                ...
                                @method('PUT')
                                <button
                                    title="{{ $order->is_done ? 'Изменить статус на "Новый"' : 'Изменить статус на "Обработан"'}}"
                                    type="submit" class="btn {{ $order->is_done ? 'btn-danger' : 'btn-primary'}}">
